...
public class SkeletonWrapper : MonoBehaviour {
	...
	void Update () {
		if (!player1tracked && !player2tracked) {
			if (trackedPlayers [0] >= 0) {
				player1tracked = true;
		...
		} else if (player1tracked && player2tracked) {
			if (trackedPlayers [0] == -1) {
				player1tracked = false;
				AliceSync.s.RemoveBody (0);
			}
			if (trackedPlayers [1] == -1) {
				player2tracked = false;
				AliceSync.s.RemoveBody (1);
			}
		}else if (!player1tracked && player2tracked) {
			if (trackedPlayers [0] >= 0) {
				player1tracked = true;
		...
		//Debug.Log (trackedPlayers [0]);
		//Debug.Log (trackedPlayers [1]);
	}
